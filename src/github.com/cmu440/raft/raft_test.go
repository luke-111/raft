...
type config struct {
	mu        sync.Mutex    // guards shared test state
	t         *testing.T    // test handle
	net       *rpc.Network  // simulated network
	n         int           // number of servers
	done      int32         // set to ask helpers to exit
	rafts     []*Raft       // running Raft instances
	applyErr  []string      // errors seen by apply readers
	connected []bool        // whether each server is connected
	endnames  [][]string    // port names each server uses
	logs      []map[int]int // committed log copies per server
}
