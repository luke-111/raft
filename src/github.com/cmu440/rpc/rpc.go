...
type reqMsg struct {
	endname  interface{}   // name of the caller
	svcMeth  string        // service.method to invoke
	argsType reflect.Type  // concrete type of the args
	args     []byte        // encoded args payload
	replyCh  chan replyMsg // channel used to get the reply
}

type replyMsg struct {
	ok    bool   // true if reply arrived
	reply []byte // raw reply bytes
}

type ClientEnd struct {
	endname interface{} // identifier for this endpoint
	ch      chan reqMsg // shared network channel
}

func (e *ClientEnd) String() string {
	return fmt.Sprintf("%v", e.endname)
}
...
type Network struct {
	mu             sync.Mutex                  // protects network state
	reliable       bool                        // whether to drop/delay messages
	longDelays     bool                        // whether to allow long sleeps
	longReordering bool                        // whether replies can be delayed a lot
	ends           map[interface{}]*ClientEnd  // all client endpoints
	enabled        map[interface{}]bool        // endpoint enabled flags
	servers        map[interface{}]*Server     // registered servers
	connections    map[interface{}]interface{} // mapping endname -> servername
	endCh          chan reqMsg                 // central request channel
}
...
type Server struct {
	mu       sync.Mutex          // protects services map
	services map[string]*Service // registered services
	count    int                 // incoming RPC counter
}
...
type Service struct {
	...
	rcvr    reflect.Value             // receiver object
	typ     reflect.Type              // receiver type
	methods map[string]reflect.Method // exported handler methods
}
...
func (svc *Service) dispatch(methname string, req reqMsg) replyMsg {
	if method, ok := svc.methods[methname]; ok {
		...
		if err := ad.Decode(args.Interface()); err != nil {
			log.Fatalf("rpc.Service.dispatch(): decode reply: %v\n", err)
		}
		...
		if err := re.EncodeValue(replyv); err != nil {
			log.Fatalf("rpc.Service.dispatch(): encode reply: %v\n", err)
		}
